...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await BecoReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.beco.address, '100', '1000', { from: owner });

        await this.beco.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setBecoReferral(this.referral.address, { from: owner });
